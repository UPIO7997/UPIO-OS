...
export default function Influencers() {
  ...
  const handleSave = async () => {
    try {
      if (editingId) {
        await updateDoc(doc(db, "influencers", editingId), formData)
      } else {
        await addDoc(collection(db, "influencers"), formData)
      }
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div className="space-y-1">
          <div>
            ...
            <h2 className="text-3xl font-bold tracking-tight">Influencer Management</h2>
          </div>
        </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="text-center py-4 text-zinc-500">No influencers found.</td></tr>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto dark:bg-[#0C0C0E] dark:border-[#27272A]">
          <DialogHeader>
            <DialogTitle className="dark:text-[#FAFAFA]">{editingId ? 'Edit Influencer' : 'Add Influencer'}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button onClick={handleSave}>{editingId ? 'Save changes' : 'Add Influencer'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
